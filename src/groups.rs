...
pub fn command(matches: &ArgMatches) {
    if let Some(_) = matches.subcommand_matches("list") {
       ...
       if let Some(business_unit) = bu {
           accounts = get_accounts_by_business_unit(business_unit, mfa);
       }
...
fn get_accounts_by_business_unit(name: &str, mfa: &str) -> Vec<Account> {
    let client = Saml2Aws::new();

    match client.list_roles(mfa) {
        Ok(a) => a,
        Err(_) => vec![],
    }.into_iter().filter(|a| a.name.starts_with(name)).collect()
}
